...
onready var CellScene = preload("res://Sudoku/Scenes/Cell.tscn")
onready var create_button = get_node("../../MenuPortrait/ButtonContainer/Create")
onready var undo_button = get_node("../../MenuPortrait/ButtonContainer/Undo")
onready var redo_button = get_node("../../MenuPortrait/ButtonContainer/Redo")
onready var check_button = get_node("../../MenuPortrait/ButtonContainer/Validate")
onready var pencil_button = get_node("../../MenuPortrait/ButtonContainer/Pencil")
onready var clear_button = get_node("../../MenuPortrait/ButtonContainer/Clear")
onready var is_sudoku_valid_button = get_node("../../MenuPortrait/IsSudokuValid")
...
func _calculate_grid_parameters():
	...
	scaled_cell_size = round(grid_dimension / grid_size)
...
func _draw():
	...
	# Draw white background for the entire grid area
	var total_grid_size = Vector2(grid_size * scaled_cell_size, grid_size * scaled_cell_size)
	draw_rect(Rect2(Vector2(start_x, start_y), total_grid_size), Global.button_bg_color, true)  # true for filled rectangle
	...
	if use_font:
		...
		for i in range(grid_size):
			for j in range(grid_size):
				var digit_value = puzzle[i][j]
				if digit_value != 0:
					var digit_str = str(digit_value)
					var cell_pos = Vector2(start_x + j * scaled_cell_size, start_y + i * scaled_cell_size)
					var cell_center = cell_pos + Vector2(scaled_cell_size / 2, scaled_cell_size / 2)

					# Calculate the position for the text to be centered in the cell
					var text_size = font.get_string_size(digit_str)
					var font_ascent = font.get_ascent() * digit_scale_factor
					var text_pos = cell_center - text_size / 2
					text_pos.y += font_ascent * 6 # Adjust this value as needed to center the text

					# Correct way to select color based on number_source
					var color
					if number_source[i][j] == "game":
						color = Global.game_placed_digit_color
					else:
						color = Global.player_digit_color
					
					draw_string(font, text_pos, digit_str, color)

					# Manually ensure the text does not render outside the cell
					# Adjust text_pos here if necessary before drawing
					draw_string(font, text_pos, digit_str, color)
...
func _process(delta):
	if mouse_button_down:
		...
		if (start_x <= mouse_position.x) and (mouse_position.x <= start_x + grid_width) and (start_y <= mouse_position.y) and (mouse_position.y <= start_y + grid_height):
			...
			# Calculate the cell indices based on mouse position
			var i = int((mouse_position.y - start_y) / scaled_cell_size)
			var j = int((mouse_position.x - start_x) / scaled_cell_size)

			# Ensure the indices are within the grid size
			if i >= 0 and i < grid_size and j >= 0 and j < grid_size:
				...
				if !selected_cells.has(cell_vector):
					...
					# Optionally, break here if you only want to select one cell at a time
	...
	if save_button_name != "Custom Game":
		...
		if all_cells_filled(puzzle):
			...
			if filled_grid.is_valid_sudoku(puzzle):
				...
				var button_path = "../../MenuPortrait/PuzzleCompleted"
				var puzzle_completed = get_node(button_path)

				puzzle_completed.visible = true
				print("Sudoku is valid")
				var audio_player = $"/root/Sudoku/PuzzleCompleteSound"
				audio_player.play()
...
func _on_Validate_button_up():
	...
	var current_grid = puzzle.duplicate(true)
#	var current_grid = [
#		[0,9,0,5,1,0,0,7,2],
#		[5,0,7,0,0,4,0,0,1],
#		[0,6,3,8,7,2,5,4,9],
